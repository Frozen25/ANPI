...
void testArithmetic() {
  ...
  {// Matrix*Matrix Test Case
    ...
    try {
      b = { {4,5},{5,4},{3,3},{1,2} };
      c = a*b;
      BOOST_CHECK(false && "Multiplication should catch bad matrix 'b' size");
    } catch(anpi::Exception exc ) {
      BOOST_CHECK(true && "successfully catched");
    }
  } 

  { // Matrix*Vector Test Case
    M a = { {1,2,3},{ 3, 2, 1} };
    std::vector<typename M::value_type> b = {5,6,7};
    std::vector<typename M::value_type> r = {38,34};
    ...
    try {
      b = { 5,6,7,8 };
      c = a*b;
      BOOST_CHECK(false && "Multiplication should catch bad vector 'b' size");
    } catch(anpi::Exception exc ) {
      BOOST_CHECK(true && "successfully catched");
    }
  } 
